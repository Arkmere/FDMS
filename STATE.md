...
- **H4 implemented:** Historic Strip Board now has a structured AND filter panel with controls for callsign, registration, pilot, aircraft type, EGOW code, WTC, flight type, departure AD, arrival AD, plus a free-text OR search across callsign/registration/type/route/pilot/remarks/EGOW/WTC/flightType. Filters apply after period/calendar date selection. Clear filters button clears structured controls only; period dropdown and calendar selected-date are unaffected. Date range filters deferred to H4b/H5 to avoid conflicting with period dropdown and calendar-selected date semantics. Export filtered CSV deferred to H5 or H4b; CSV export continues to use existing period logic.
