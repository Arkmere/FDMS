...
- **H4 implemented:** Historic Strip Board now has a structured AND filter panel with controls for callsign, registration, pilot, aircraft type, EGOW code, EGOW unit code, WTC, flight type, departure AD, arrival AD, plus a free-text OR search across callsign/registration/type/route/pilot/remarks/EGOW/WTC/flightType. Filters apply after period/calendar date selection. Clear filters button clears structured controls only; period dropdown and calendar selected-date are unaffected. Date range filters deferred to H4b/H5 to avoid conflicting with period dropdown and calendar-selected date semantics. Export filtered CSV deferred to H5 or H4b; CSV export continues to use existing period logic. H4 corrective patch applied: registration matching normalises punctuation/hyphens so G-GORV and GGORV match equivalently; pilot/PIC matching searches all known pilot, PIC, attribution, and formation element identity fields; EGOW unit code filter added with options No filter / Any unit code / L / M / A using token-based matching.
- **H5 implemented:** Search / Table view now provides independent structured search across completed movement history with a dedicated filter panel (date from, date to, free text, callsign, registration, pilot, aircraft type, EGOW code, EGOW unit code, WTC, flight type, departure AD, arrival AD). Date from / Date to use lexical YYYY-MM-DD comparison against m.dof; blank = no date bound. Normalised registration matching (G-GORV matches GGORV), widened pilot/PIC/attribution matching, and EGOW unit-code token matching are all delegated to the shared H4 helpers (movementMatchesHistoryFilters). Table shows 15 columns: Date, Callsign, Registration, Type, WTC, Flight type, Dep AD, Arr AD, Times, EGOW code, EGOW unit, Pilot, Activity, Status, Actions. Results default to all completed movements sorted newest first. Result count is displayed. Open info reuses renderExpandedRow with 'history' context. Jump to day sets historyCalendarSelectedDate and switches to Historic Strip Board via the extracted activateMovementHistoryView() helper. Export filtered CSV exports currently filtered H5 rows only to history-search-results-YYYY-MM-DD.csv; existing Historic Strip Board CSV export is unchanged. Column-click sorting deferred to H6. Pagination deferred to H5b/H6.
